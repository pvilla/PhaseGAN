...
class TrainModel(ABC):
    ...
    def plot_cycle(self, img_idx, epoch,i, layer=0, test=False, plot_phase=True):
        ...
        for i in l:
            if test == True:
                im = axs[i].imshow(getattr(self,self.img_names[i])[img_idx, 0, :, :].cpu())
            else:
                im = axs[i].imshow(getattr(self,self.img_names[i])[img_idx, 0, :, :].detach().cpu())
            axs[i].axis("off")
            axs[i].set_title(img_list[i], fontsize=36)
        for i in ignore:
            if test == True:
                im = axs[i].imshow(getattr(self,self.img_names[i])[img_idx, layer, :, :].cpu())
            else:
                im = axs[i].imshow(getattr(self,self.img_names[i])[img_idx, layer, :, :].detach().cpu())
            axs[i].axis("off")
            axs[i].set_title(img_list[i], fontsize=36)
        if save_name != 0:
            if test is True:
